...
def get_prefix(bot, message):
    try:
        ...
        with open('prefixes.json', 'r') as k:
            config = json.load(k)
        config['prefixes'][str(message.guild.id)] = '+'
        ...
        return config['prefixes']['default']  # This will return "+" as a prefix. You can change it to any default prefix.
...
owners = [908234266301829132, 829619198544183327, 711828107061100615]

class Bot(commands.Bot):

    def __init__(self, intents,get_msg_prefix,**kwargs):
        super().__init__(
            command_prefix=get_msg_prefix,
            owner_ids=set([908234266301829132, 829619198544183327, 711828107061100615]),
...
async def random_fact(ctx):
    ...
    embed = discord.Embed(title="Fact!", description=f"{fact}")
