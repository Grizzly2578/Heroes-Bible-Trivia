...
class Song:
    ...
    def create_embed(self):
        ...
        embed.set_footer(text=f"Grizzly Music by: Grizzly.#8401", icon_url="https://cdn.discordapp.com/avatars/908234266301829132/3c3f46e5b1f1db3673876d97a1833247.gif")
...
class Music(commands.Cog):
    ...
    @commands.command(
        name='summon',
        description=
        "Summons the bot to a voice channel. \nIf no channel was specified, it joins your channel. \n *Requires `Manage Guilds` Permission!*"
    )
    ...
    async def _now(self, ctx: commands.Context):
        ...
        await ctx.send(embed=ctx.voice_state.current.voice_state())
    ...
    @commands.command(name='play', description=f"Play's a Song from YouTube")
    ...
    @commands.group(name="prefix",description="Prefix Settings.", invoke_without_command=True)
    @commands.guild_only()
    async def prefix(self, ctx:commands.Context):
      embed=discord.Embed(title="Prefix", description=f"Current prefix: `{get_prefix(bot, ctx)}`\n\n**Commands:**\n\n**Set** Set's a prefix for the server.", color=0x2f3136)
    ...
    async def set(self, ctx, prefix):  #command: bl!changeprefix ...
    ...
    async def set_error(self, ctx:commands.Context, error):
      if isinstance(error, commands.MissingPermissions):
        await ctx.reply(content="You lack permissions to use this command!", mention_author=False)
      if isinstance(error, commands.MissingRequiredArgument):
        await ctx.reply(content="Missing Argument: No prefix provided.", mention_author=False)
...
#Help_Command
    @bot.command(name='help', description="Shows The List of Commands.")
    async def help(ctx: commands.Context):
        embed = discord.Embed(
            title=f"THE COMMANDS",
            description=
            f"Did you know that you can activate commands by mentioning the bot's name? For example, type {get_prefix(bot, ctx)} **help** to show the list you are seeing now.",
            color=0x2f3136)
        for command in bot.walk_commands():
          if command.parent == None:
            description = command.description
            if not description or description is None or description == "":
                description = f"No Description Provided ask Grizzly to put descriptions lol"
            embed.add_field(
                name=f"{command.name}",
                value=
                f"<:ReplyD:979001953789042739> {description} `{get_prefix(bot, ctx)}` **{command.name}**",
                inline=False)
            embed.set_thumbnail(
                url=
                f"https://cdn.discordapp.com/emojis/766362973446078464.gif?size=4096&quality=lossless"
            )
